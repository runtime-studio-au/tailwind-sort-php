 ...
 *
 *   - Only the *value* of a `key => value` pair is eligible; array keys are never sorted (decided by position
 *     relative to `=>`, never by inspecting the string's content).
...
export function findSortablePhpStrings(src: string, islands: Island[]): PhpStringRange[] {
  ...
  for (const isl of islands) {
    const toks = tokenizeIsland(src, isl.start, isl.end);
    for (let k = 0; k < toks.length; k++) {
      const tok = toks[k];
      if (tok.kind !== 'string' || tok.skip) continue;
...
/**
 * Tokenize one PHP island into the minimal token stream needed for value classification: string literals (with
 * their inner range and a skip flag), the `=>` arrow, the `.` concatenation operator, and a coalesced `other`
 * marker for everything else. Whitespace and comments are dropped so adjacency is judged across them.
 */
function tokenizeIsland(src: string, start: number, end: number): Tok[] {
  const toks: Tok[] = [];
  const pushOther = () => {
    if (toks.length === 0 || toks[toks.length - 1].kind !== 'other') toks.push({ kind: 'other' });
  };
  ...
  while (i < end) {
    ...
    if (c === "'") {
      ...
      toks.push({ kind: 'string', start: i + 1, end: close, skip: inner.includes('\\') });
      i = close + 1;
      continue;
    }
    if (c === '"') {
      ...
      toks.push({ kind: 'string', start: i + 1, end: close, skip: hasInterpolationOrEscape(inner) });
      i = close + 1;
      continue;
    }
    if (c === '`') {
...
/**
 * Skip a heredoc/nowdoc; `i` is just past `<<<`. Returns the offset past the closing identifier line, or -1 if
 * `<<<` isn't followed by a valid heredoc identifier. Mirrors the boundary rules of the island lexer.
 */
