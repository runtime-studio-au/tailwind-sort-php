 ...
 *
 * Builds the real Tailwind sort function against a fixture Tailwind v4 stylesheet and runs the island-aware transform
 * with the real engine — proving the official sort order and that custom theme and utility vocabulary is recognized,
 * which the mock-sorter suite cannot. The whole suite skips when the optional Tailwind toolchain isn't installed,
 * so the core tests stay dependency-free.
 */
...
describe('real Tailwind sorter integration', { skip }, () => {
    test('canonicalizes any input order and recognizes custom utilities', () => {
        const tokens = ['section-py', 'z-10', 'flex', 'bg-brand', 'p-4', 'm-2'];
        const sorted = sortFn!(tokens);
        // Every permutation yields the same order, so each token (including the custom `section-py` and `bg-brand`)
        // has a known sort position; unrecognized classes would instead keep their input order.
